 ...
 */
package top.jessi.okgo.callback;
 ...
 * ---网络请求失败 onStart -> onFailure -> onFinish<br>
 ...
 * ---网络请求失败,读取缓存失败 onStart -> onFailure -> onFinish<br>
 ...
 * ---没有缓存请求网络失败 onStart -> onFailure -> onFinish<br>
 ...
 * ---无缓存时,网络请求失败 onStart -> onFailure -> onFinish<br>
 * ---有缓存时,网络请求成功 onStart -> onCacheSuccess -> convertResponse -> onSuccess -> onFinish<br>
 * ---有缓存时,网络请求失败 onStart -> onCacheSuccess -> onFailure -> onFinish<br>
 */
public interface Callback<T> extends Converter<T> {
    ...
    void onFailure(@NonNull Response<T> response);
